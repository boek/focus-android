...
class PrivacySecuritySettingsFragment : BaseSettingsFragment(),
    ...
    private fun updateBiometricsToggleAvailability() {
        val switch = preferenceScreen.findPreference(resources.getString(R.string.pref_key_biometric)) as SwitchPreferenceCompat

        if (!Biometrics.hasFingerprintHardware(requireContext())) {
            switch.isChecked = false
            switch.isEnabled = false
            preferenceManager.sharedPreferences.edit().putBoolean(resources.getString(R.string.pref_key_biometric), false).apply()
        } else {
